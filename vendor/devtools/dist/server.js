...
import { inspectMcpOAuth } from "@senda/mcp";
import { AdapterBusyError, isAdapterId } from "./adapter-runner.js";
import { EntryTargetError, isEntryAdapter, parseEntryPoint, } from "./entry-target.js";
...
<title>Senda DevTools · Project workspace</title>
${faviconLink}
<style>
...
`;
const oauthCallbackCopy = {
    success: [
        "Authorization complete",
        "Return to Senda DevTools. You can close this tab.",
    ],
    rejected: [
        "Authorization was not completed",
        "Return to Senda DevTools to try again.",
    ],
    invalid: [
        "Authorization callback was invalid",
        "Return to Senda DevTools and start the authorization again.",
    ],
    error: [
        "Authorization failed",
        "Return to Senda DevTools to review the endpoint.",
    ],
};
function oauthCallbackPage(outcome) {
...
function isPrincipalInput(value) {
...
function readCapabilityView(value) {
    if (typeof value !== "object" || value === null)
        return undefined;
    const record = value;
 ...
 */
function toAdapterCapture(result, principalId, input) {
    const shared = {
    ...
    return {
        ...
        response: result.exchange.stderr === ""
 ...
 */
export async function startDevtoolsServer(options) {
    const uiRoot = options.uiRoot ?? defaultUiRoot();
    const engineEndpoint = () => `${literalLoopbackOrigin(options.enginePort())}/mcp`;
    ...
    const sendTraceFrame = (response, entry) => {
    ...
    const serveStatic = (response, relativeSegments) => {
    ...
    const handlePrincipals = async (request, response) => {
        ...
        const body = parseJson(read.body);
        ...
        sendJson(response, 405, { error: "method_not_allowed" }, { allow: "GET, POST, PUT, DELETE" });
    };
     ...
     */
    const handleHttpTargetCheck = async (request, response) => {
        const read = await readBody(request, apiBodyLimitBytes);
        if (!read.ok) {
            sendJson(response, 413, { error: "payload_too_large" });
            return;
        ...
        if (typeof body?.url === "string" && body.url !== "") {
            ...
            if (target.kind !== "external") {
                sendJson(response, 400, {
                    ...
                    message: "The devtools host authenticates with its own session tokens; there is no OAuth chain to check.",
                });
                return;
            }
            url = target.url;
     ...
     */
    const handleHttpTarget = async (request, response) => {
        if (request.method === "GET") {
        ...
        if (request.method !== "PUT" && request.method !== "POST") {
            sendJson(response, 405, { error: "method_not_allowed" }, { allow: "GET, PUT, POST, DELETE" });
            return;
        }
        const read = await readBody(request, apiBodyLimitBytes);
        if (!read.ok) {
     ...
     */
    const handleEntryTarget = async (request, response) => {
        if (request.method === "GET") {
        ...
        if (request.method !== "PUT") {
            sendJson(response, 405, { error: "method_not_allowed" }, { allow: "GET, PUT, DELETE" });
            return;
        }
        const read = await readBody(request, apiBodyLimitBytes);
        if (!read.ok) {
        ...
        try {
            options.entryTarget.set(body.adapter, parseEntryPoint(body.entryPoint, options.cwd));
        }
        catch (error) {
    ...
    const handleOAuthCallback = async (request, response, url) => {
        ...
        const rejectCallback = async (outcome) => {
            try {
        ...
        if (error !== undefined) {
            await rejectCallback(error.length === 0 || error.length > 256 ? "invalid" : "rejected");
            return;
        }
        const code = codes[0];
     ...
     */
    const handleInvoke = async (request, response) => {
        ...
        options.trace.appendAdapterCall(toAdapterCapture(result, identity?.principal.id ?? null, JSON.stringify(body.arguments ?? {})));
        sendJson(response, 200, result);
    };
    const handleMcpProxy = async (request, response) => {
        const read = await readBody(request, mcpBodyLimitBytes);
        if (!read.ok) {
            sendJson(response, 413, { error: "payload_too_large" });
            return;
        ...
        const requestJson = parseJson(read.body);
        const mcpMethod = typeof requestJson?.method === "string" ? requestJson.method : undefined;
        const capabilityId = mcpMethod === "tools/call" &&
    ...
    const handle = async (request, response) => {
        ...
        if (path === "/oauth/result") {
            ...
            response.end(oauthCallbackPage(outcome === "success" ||
                outcome === "rejected" ||
                outcome === "invalid"
                ? outcome
                : "error"));
