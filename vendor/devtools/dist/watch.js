...
export async function startWatchMode(options) {
    ...
    const watchRoots = (options.include === undefined || options.include.length === 0
        ? [options.cwd]
        : options.include).map((includePath) => resolve(options.cwd, includePath));
    let child;
    let snapshot;
    let closed = false;
    ...
    const sendPrincipals = () => {
        const records = options.principals
            .list()
            .map(({ token, principal }) => ({ token, principal }));
        try {
            child?.stdin?.write(`${JSON.stringify({ type: "principals", records })}\n`);
        }
        catch {
    ...
    const startChild = () => new Promise((resolvePromise) => {
        let settled = false;
        const settle = (result) => {
            if (settled)
                return;
        ...
        spawned.stderr.on("data", (chunk) => {
            ...
            for (const line of lines) {
                ...
                let message;
                try {
                    message = JSON.parse(line.slice(protocolPrefix.length));
                }
                catch {
                    continue;
                }
                ...
                if (message.type === "record") {
                    options.trace.appendInvocation(message.record);
                    continue;
                }
    ...
    const runBuild = () => new Promise((resolvePromise) => {
    ...
    const buildDetail = (output) => output.split("\n").slice(0, noticeDetailLineLimit).join("\n").trimEnd();
    const runCycle = async () => {
        ...
        do {
            ...
            if (started.kind === "ready") {
                ...
                diagnostic(`rebuild ok (${elapsed}s), engine restarted on port ${String(started.snapshot.port)}\n`);
            }
            else {
                options.trace.appendNotice("engine-start-failed");
                diagnostic("senda-devtools: the rebuilt engine could not start; waiting for the next change.\n");
            }
        } while (dirty && !closed);
        cycleRunning = false;
    };
    ...
    const handleWatchEvent = (root) => (_event, filename) => {
        if (filename === null) {
            schedule();
            return;
        }
        ...
        if (changedPath === modulePath)
            return;
        if (moduleDirectoryInsideCwd &&
    ...
    const watchers = watchRoots
        ...
        if (existsSync(root))
            return true;
        diagnostic(`senda-devtools: watch path does not exist, skipping: ${root}\n`);
        return false;
    })
        .map((root) => watchDirectory(root, { recursive: true }, handleWatchEvent(root)));
    return {
        kind: "started",
        handles: {
            engineView: () => ({
