...
function capabilityFromHash() {
    try {
        ...
        if (route !== "capabilities" || rest.length === 0)
            return undefined;
 ...
 */
export function focusCapabilitySearch() {
    if (mountedSearch === undefined)
        return false;
    mountedSearch.focus();
...
function renderDetail(capability) {
    const annotations = Object.entries(capability.annotations ?? {})
        ...
        .map(([name]) => name);
    const metadata = annotations.map((name) => el("span", { class: "badge" }, [name]));
    if (capability.timeoutMs !== undefined) {
        metadata.push(el("span", { class: "badge" }, [
    ...
    return el("div", { class: "capability-detail" }, [
        el("header", { class: "capability-overview" }, [
            ...
            metadata.length === 0
                ? null
                : el("div", { class: "capability-meta", "aria-label": "Capability metadata" }, metadata),
        ]),
...
export function renderCapabilitiesPanel(container) {
    let active = true;
    let loading = false;
    let ownedSearch;
    ...
    let selectCapabilityById;
    const selectListener = (event) => {
        const id = event.detail?.id;
        if (typeof id === "string")
            selectCapabilityById?.(id);
    };
    const documentListens = typeof document.addEventListener === "function";
    if (documentListens) {
        document.addEventListener("senda:select-capability", selectListener);
    }
    const load = () => {
        if (loading)
            return;
        loading = true;
        clear(container);
        container.append(el("h2", {}, ["Capabilities"]), el("p", { class: "hint", role: "status" }, [
        ...
        void api
            ...
            if (capabilities.length === 0) {
                container.append(el("h2", {}, ["Capabilities"]), el("p", { class: "empty", role: "status" }, [
            ...
            let selectedButton;
            const select = (capability, button) => {
            ...
            for (const [index, capability] of capabilities.entries()) {
                ...
                button.addEventListener("keydown", (event) => {
                    const visibleChoices = choices.filter(({ button: candidate }) => !candidate.hidden);
                    const currentIndex = visibleChoices.findIndex(({ button: candidate }) => candidate === button);
                    if (currentIndex === -1 || visibleChoices.length === 0)
                        return;
                    let nextIndex;
            ...
            const filterEmpty = el("p", { class: "empty capability-filter-empty", role: "status" }, [
            ...
            selectCapabilityById = (id) => {
                ...
                if (match === undefined || match.button === selectedButton)
                    return;
                search.value = "";
                applyFilter();
            ...
            const sidebar = el("aside", {
                ...
                el("h2", { id: sidebarHeadingId, class: "capability-sidebar-heading" }, ["Capabilities"]),
                count,
            ...
            container.append(el("div", { class: "capabilities-layout" }, [sidebar, detail]));
            // A shared `#capabilities/<id>` link opens on its capability.
            const deepLinked = capabilityFromHash();
            const initial = (deepLinked === undefined
                ? undefined
            ...
            if (!active)
                return;
            clear(container);
            const retry = el("button", { type: "button", class: "capability-retry" }, ["Retry"]);
            retry.addEventListener("click", () => {
                load();
            });
            container.append(el("h2", {}, ["Capabilities"]), el("p", { class: "feedback", role: "alert" }, [
    ...
    return () => {
        ...
        if (mountedSearch === ownedSearch)
            mountedSearch = undefined;
        if (documentListens && typeof document.removeEventListener === "function") {
            document.removeEventListener("senda:select-capability", selectListener);
        }
    };
}
