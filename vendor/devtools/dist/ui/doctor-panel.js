...
const compositionCheckCommand = "senda check-capabilities";
const restartRecheckDelayMs = 250;
function readString(diagnostic, property) {
    const value = diagnostic[property];
    return typeof value === "string" ? value : undefined;
}
function readErrorMessage(diagnostic) {
...
function describeDiagnostic(kind, diagnostic) {
    ...
    switch (readString(diagnostic, "code")) {
        ...
        case "DESCRIBE_FAILED":
            return `${capabilityId ?? "Unknown capability"} — ${errorMessage ?? "The capability description could not be read."}`;
        ...
        case "MCP_MANIFEST_PRESENT":
            return "senda.mcp.json is present in the working directory.";
        case "MCP_MANIFEST_MISSING":
            return "No senda.mcp.json was found in the working directory.";
...
function renderDiagnostic(kind, diagnostic, index) {
    ...
    if (code === "COMPOSITION_CHECK_AVAILABLE") {
        actions.push(createCopyButton("command", () => compositionCheckCommand, "copy-button"));
    }
...
function renderDiagnosticSection(kind, diagnostics) {
    const isFinding = kind === "finding";
    const title = isFinding ? "Findings" : "Notes";
    const id = `doctor-${isFinding ? "findings" : "notes"}-title`;
    const count = diagnostics.length;
    return el("section", {
        class: `diagnostic-section diagnostic-section--${kind}${count === 0 ? " diagnostic-section--empty" : ""}`,
        "aria-labelledby": id,
    }, [
        ...
        count === 0
            ...
            }, diagnostics.map((diagnostic, index) => renderDiagnostic(kind, diagnostic, index))),
    ]);
}
function renderReport(report) {
    ...
    return el("div", { class: "doctor-report" }, [
        el("div", {
            class: `doctor-summary doctor-summary--${healthy ? "healthy" : "issues"}`,
        }, [
...
export function renderDoctorPanel(container) {
    let active = true;
    let loading = false;
    clear(container);
    ...
    const reportContent = el("div", { class: "doctor-content", id: "doctor-report" }, []);
    ...
    const runChecks = async (reason) => {
        ...
        try {
            ...
            status.setAttribute("class", `doctor-state badge ${healthy ? "ok" : "error"}`);
...
//# sourceMappingURL=doctor-panel.js.map