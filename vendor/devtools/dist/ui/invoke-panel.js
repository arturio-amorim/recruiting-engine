import { ApiError, invokeCapability, toolCallRequest, } from "./api.js";
import { createAdapterSelector, getSelectedAdapter, presentationFor, } from "./adapters.js";
 ...
 */
const handoffTargets = new Map();
let handoffListenerAttached = false;
function ensureHandoffListener() {
    if (handoffListenerAttached)
        return;
    if (typeof document.addEventListener !== "function")
        return;
    handoffListenerAttached = true;
    document.addEventListener("senda:select-capability", (event) => {
...
const tokenVariableName = "SENDA_DEV_TOKEN";
/**
...
function fieldIssue(name, value, schema) {
    ...
    if (Array.isArray(record.enum) && record.enum.length > 0) {
        const allowed = record.enum.some((candidate) => pretty(candidate) === pretty(value));
        if (!allowed)
            return `"${name}" must be one of the allowed values.`;
    }
    return undefined;
}
/** Surfaces the SyntaxError position info instead of a bare "invalid JSON". */
function syntaxDetail(error) {
...
export function renderInvokePanel(capability) {
    ...
    const windowBar = (label, ...actions) => el("div", { class: "code-window-bar" }, [
    ...
    const pane = (heading, copyLabel, ariaLabel, text) => {
     ...
     */
    const showExchange = (exchange) => {
        lastExchange = exchange;
        while (exchangeBody.firstChild)
            exchangeBody.firstChild.remove();
        if (exchange.kind === "http") {
            exchangeBody.append(pane(`Request · ${exchange.method} /mcp`, "raw MCP request", "Raw MCP request body", exchange.requestBody), 
            ...
            ]), "raw MCP response", "Raw MCP response body", exchange.responseBody));
        }
            ...
            exchangeBody.append(pane(exchange.transport === "stdio" ? "Server command" : "Endpoint", exchange.transport === "stdio" ? "command" : "endpoint", exchange.transport === "stdio"
                ? "Adapter command"
                : "Adapter endpoint", exchange.target), pane("Request · tools/call", "tools/call request", "Raw MCP request body", exchange.request), pane("Response · tools/call result", "tools/call response", "Raw MCP response body", exchange.response));
        }
        else {
            exchangeBody.append(pane("Command", "command", "Adapter command", exchange.command), pane(el("h4", {}, [
                "Standard output",
                el("span", { class: "raw-response-status" }, [
            ...
            ]), "standard output", "Adapter standard output", exchange.stdout));
            if (exchange.stderr !== "") {
                exchangeBody.append(pane("Standard error", "standard error", "Adapter standard error", exchange.stderr));
            }
        }
        exchangeSection.hidden = false;
    };
    const showResult = (state, content, isError = false, durationMs) => {
    ...
    let timedOut = false;
    let adapter = getSelectedAdapter();
    const clientTimeoutMs = () => (capability.timeoutMs ?? defaultTimeoutMs) +
    ...
    let lockAdapterSwitch = () => undefined;
    ...
    const applyResult = (settled) => {
        ...
        if (settled.outcome === "capability-error") {
            ...
            showResult("engine error", pretty(error ?? settled.result), true, settled.durationMs);
            return;
        }
        ...
        showResult(error?.code === "TIMEOUT" ? "timeout" : "adapter error", pretty(error ?? "The adapter reported no detail."), true, settled.durationMs);
    };
    const runInvocation = () => {
        if (pending)
            return;
        ...
        void invokeCapability({
            ...
            if (cancelledByUser) {
                ...
                showResult("cancelled", "The invocation was cancelled before a response arrived.", true, elapsedMs);
                return;
            }
            if (timedOut) {
                ...
                showResult("timeout", "No response received before the client timeout.", true, elapsedMs);
                return;
            }
    ...
    const readCurlCommand = () => {
        ...
        if (httpTarget.authentication.type === "session-token" &&
            getActiveToken() !== null) {
    ...
    const curlCopy = createCopyButton(`curl command (reads the token from $${tokenVariableName})`, readCurlCommand);
    /** The copy reproduces only a devtools-host call; anything else hides it. */
    const paintCurl = () => {
        curlCopy.hidden =
            adapter !== "mcp-http" || getHttpTarget().kind !== "devtools";
    };
    const commandCopy = createCopyButton("adapter command", () => lastExchange === undefined || lastExchange.kind === "http"
        ? ""
    ...
    const adapterNote = el("p", { class: "adapter-note", "aria-live": "polite" }, []);
    const identityNote = el("span", { class: "identity-note", "aria-live": "polite" }, []);
     ...
     */
    const identityApplies = () => {
        if (adapter !== "mcp-http")
            return entryPoint.identityApplies();
        const target = getHttpTarget();
        return (target.kind === "devtools" &&
    ...
    const applyAdapter = (next) => {
        ...
        entryPoint.show(next === "cli" ? "cli" : next === "mcp-stdio" ? "mcp-stdio" : undefined);
        paintIdentity();
    };
    ...
    const requestPane = el("section", { class: "invoke-request" }, [
        ...
        el("div", { class: "code-window" }, [
            windowBar("Capability arguments", createCopyButton("arguments", () => editor.value), curlCopy, commandCopy),
            editor,
        ]),
    ...
    const resultPane = el("section", { class: "invoke-result" }, [
        ...
        el("div", { class: "code-window result-window" }, [
            windowBar(resultState, createCopyButton("capability result", () => {
...
//# sourceMappingURL=invoke-panel.js.map