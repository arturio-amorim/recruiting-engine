import { cliAnnotationTags, cliControlId, cliErrorMessage, cliLabelFor, cliTextInput, } from "./cli-components.js";
import { nextRovingIndex, parseRunInput, seedCliInput, } from "./cli-contract.js";
...
function filterCommands(items, query) {
    ...
    if (needle === "")
        return items;
    return items.filter((item) => [item.id, item.title ?? "", item.description].some((value) => value.toLowerCase().includes(needle)));
}
export function createCliCommandsPanel(options) {
    ...
    let visible = [];
    let choiceButtons = [];
    const focusChoice = (id) => {
        Array.from(options.root.querySelectorAll(".att-tool-choice"))
            .find((candidate) => candidate.dataset.commandId === id)
            ?.focus();
    };
    const selectCommand = (id, focus) => {
    ...
    const detail = el("section", { id: "cli-command-detail", class: "att-tool-detail" }, [
        el("header", { class: "att-tool-header" }, [
...
//# sourceMappingURL=cli-command-view.js.map