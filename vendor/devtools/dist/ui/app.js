...
import { focusCapabilitySearch, renderCapabilitiesPanel, } from "./capabilities.js";
...
function syncHash(name) {
    try {
        ...
        if (name === "capabilities" && current.startsWith("capabilities/"))
            return;
...
function boot() {
    ...
    const nav = el("nav", { class: "tabs" }, tabs.map((tab) => {
    ...
    nav.addEventListener("keydown", (event) => {
        ...
        const current = tabs.findIndex(({ name }) => tabButtons.get(name) === keyboardEvent.target);
        if (current < 0)
            return;
        let next = current;
    ...
    const capabilityCount = el("span", { class: "meta-pill engine-capability-count" }, ["Capabilities …"]);
    ...
    const brand = el("div", { class: "brand-lockup" }, [
        createBrandMark(),
        el("span", { class: "brand-name" }, ["senda"]),
        el("span", { class: "product-name" }, ["DevTools"]),
    ]);
    const rails = el("div", { class: "blueprint-rails", "aria-hidden": "true" }, Array.from({ length: 5 }, () => el("span", {}, [])));
    ...
    const context = el("section", { class: "workspace-context", "aria-labelledby": title.id }, [
    ...
    document.body.append(el("div", { class: "app-shell" }, [rails, topbar, context, main]));
    ...
    const shortcutEntry = (keys, action) => el("div", { class: "shortcuts-entry" }, [
        el("dt", {}, [el("kbd", {}, [keys])]),
        el("dd", {}, [action]),
    ]);
