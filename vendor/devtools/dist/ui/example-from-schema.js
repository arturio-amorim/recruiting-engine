...
function build(schema, root, depth, seenRefs) {
    if (schema === undefined || depth > maxDepth)
        return null;
    const ref = schema.$ref;
    if (typeof ref === "string") {
        if (seenRefs.has(ref))
            return null;
        return build(resolveRef(root, ref), root, depth + 1, new Set([...seenRefs, ref]));
    }
    ...
    if (Array.isArray(allOf) && allOf.length > 0) {
        ...
        for (const branch of allOf) {
            ...
            if (branchSchema !== undefined)
                Object.assign(merged, branchSchema);
        }
        delete merged.allOf;
        return build(merged, root, depth + 1, seenRefs);
    }
...
//# sourceMappingURL=example-from-schema.js.map