...
import { getActivePrincipalKey, listKnownPrincipals, onPrincipalChange, setActivePrincipal, } from "./principals.js";
/**
...
export function resetIdentitySelects() {
    painters.clear();
}
export function createIdentitySelect(owner) {
    const select = el("select", { class: "identity-select", "aria-label": "Identity" }, []);
    const paint = () => {
        ...
        select.append(el("option", { value: anonymousValue }, ["Anonymous (no principal)"]));
