...
function exchangeBody(label, body, truncated, originalSize) {
    const heading = el("h4", { class: "field-label" }, truncated
        ? [
            `${label} — `,
...
function entrySubject(kind, value) {
...
function actingIdentity(principalId) {
...
function playgroundPrefill(requestBody) {
    try {
...
function renderEntry(entry) {
    if (entry.kind === "invocation" && entry.invocation !== undefined) {
        ...
        const outcome = invocation.outcome === "completed"
            ? "Completed"
    ...
    if (entry.kind === "exchange" && entry.exchange !== undefined) {
        ...
        const playground = capabilityId === undefined
            ? null
            : el("button", {
                type: "button",
        ...
        playground?.addEventListener("click", () => {
            if (capabilityId === undefined)
                return;
            openCapabilityInPlayground(capabilityId, playgroundPrefill(exchange.requestBody));
        });
        return el("details", { class: "trace-row exchange trace-row--exchange" }, [
            el("summary", { title: "Show raw request and response bodies" }, [
                ...
                entrySubject("MCP exchange", exchange.capabilityId ?? exchange.mcpMethod ?? "Unknown method"),
            ...
            el("div", { class: "trace-payload-grid" }, [
                exchangeBody("Request body", exchange.requestBody, entry.requestTruncated === true, entry.requestOriginalSize ??
                    (entry.requestTruncated === true ? entry.originalSize : undefined)),
                exchangeBody("Response body", exchange.responseBody, entry.responseTruncated === true, entry.responseOriginalSize ??
    ...
    if (entry.kind === "adapter" && entry.call !== undefined) {
        ...
        playground.addEventListener("click", () => {
            ...
            openCapabilityInPlayground(call.capabilityId, call.input ?? playgroundPrefill(call.request));
        });
        const tone = call.outcome === "success"
            ? "ok"
            : call.outcome === "capability-error"
                ? "error"
                : "warn";
        const outcomeLabel = call.outcome === "success"
            ? "Success"
        ...
        return el("details", { class: "trace-row adapter trace-row--adapter" }, [
            ...
            el("div", { class: "trace-row-actions" }, [playground]),
            el("div", { class: "trace-payload-grid" }, [
                exchangeBody("Request", call.request, entry.requestTruncated === true, entry.requestOriginalSize),
                exchangeBody("Response", call.response, entry.responseTruncated === true, entry.responseOriginalSize),
            ]),
        ]);
    }
...
function searchTextOf(entry) {
    const parts = [entry.kind, entry.at];
    if (entry.invocation !== undefined) {
        parts.push(entry.invocation.capabilityId, entry.invocation.outcome, entry.invocation.errorCode ?? "");
    }
    if (entry.exchange !== undefined) {
        parts.push(`http ${String(entry.exchange.status)}`, entry.exchange.mcpMethod ?? "", entry.exchange.capabilityId ?? "", entry.exchange.requestBody, entry.exchange.responseBody);
    }
    if (entry.call !== undefined) {
        parts.push(entry.call.adapter, entry.call.capabilityId, entry.call.outcome, entry.call.errorCode ?? "", entry.call.principalId === undefined
            ? ""
            : (entry.call.principalId ?? "anonymous"), entry.call.status === undefined
            ? ""
            : `http ${String(entry.call.status)}`, entry.call.request, entry.call.response);
    }
 ...
 */
export function renderTracePanel(container) {
    ...
    const warning = el("aside", { id: "trace-data-warning", class: "trace-safety-note", role: "note" }, [
    ...
    const filterEmpty = el("div", { class: "trace-empty trace-filter-empty", role: "status" }, [
    ...
    const count = el("span", { id: "trace-count", class: "trace-count", "aria-hidden": "true" }, [entryCount(0)]);
    const list = el("div", {
        id: "trace-list",
        class: "trace-list",
        role: "log",
    ...
    const errorsOnlyToggle = el("button", {
        type: "button",
    ...
    const applyFilter = () => {
        ...
        for (const record of records) {
            const matches = (activeKind === "all" || record.kind === activeKind) &&
    ...
    const admit = (key, entry) => {
        const capabilityId = entry.invocation?.capabilityId ??
    ...
    function selectKind(value, focus) {
        activeKind = value;
    ...
    filterGroup.addEventListener("keydown", (event) => {
        ...
        if (keyboardEvent.key === "ArrowRight" || keyboardEvent.key === "ArrowDown")
            ...
            next = kindFilters.length - 1;
        else
            return;
        keyboardEvent.preventDefault();
        const choice = kindFilters.at((next + kindFilters.length) % kindFilters.length);
        if (choice !== undefined)
            selectKind(choice.value, true);
    });
    ...
    const panel = el("section", { class: "trace-panel", "aria-labelledby": "trace-heading" }, [
        ...
        warning,
        el("section", { class: "trace-feed", "aria-labelledby": "trace-feed-heading" }, [
    ...
    const cancelReplaySettlement = () => {
        if (settleTimer === undefined)
            return;
        clearTimeout(settleTimer);
        settleTimer = undefined;
    };
    const scheduleReplaySettlement = () => {
        if (!connected || !replayPending)
            return;
