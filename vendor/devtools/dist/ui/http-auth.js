...
function isHttpTargetView(value) {
    if (typeof value !== "object" || value === null)
        return false;
    const record = value;
    if (record.kind !== "devtools" && record.kind !== "external")
        return false;
    return (typeof record.authentication === "object" &&
        record.authentication !== null &&
        typeof record.authentication.type ===
            "string");
}
export async function loadHttpTarget() {
    if (loaded)
        return;
    loaded = true;
    try {
        const view = await api.httpTarget();
        if (!isHttpTargetView(view))
            return;
        current = view;
        publish();
    }
...
export function describeHttpTarget(target) {
    ...
    const authentication = type === "oauth"
...
export function createHttpAuthControl(owner, onTargetChange) {
    const summary = el("span", { class: "auth-summary" }, [
        describeHttpTarget(current),
    ]);
    const choice = el("select", { class: "auth-choice", "aria-label": "Authentication" }, [
    ...
    const feedback = el("p", { class: "auth-feedback", "aria-live": "polite" }, []);
    const url = el("input", {
        type: "url",
        class: "auth-url",
    ...
    const externalType = el("select", { class: "auth-external-type", "aria-label": "Credential" }, [
    ...
    const checkReport = el("div", { class: "auth-check-report", role: "status", "aria-live": "polite" }, []);
     ...
     */
    const showCheck = (inspection, checkedUrl) => {
        while (checkReport.firstChild)
            checkReport.firstChild.remove();
        for (const step of inspection.steps) {
            const tone = step.outcome === "ok"
                ? "ok"
                : step.outcome === "failed"
                    ? "error"
                    : "warn";
            checkReport.append(el("div", { class: "auth-check-step" }, [
    ...
    const send = async (body) => {
        ...
        try {
            ...
            for (let attempt = 0; attempt < authorizationPollAttempts; attempt += 1) {
                await new Promise((resolve) => setTimeout(resolve, authorizationPollMs));
    ...
    apply.addEventListener("click", () => {
        const type = externalType.value;
        const authentication = type === "bearer"
