import { buildCliTarget, clearCliSecrets, completeConnectionAttempt, retainedActivityOf, TargetDraftValidationError, } from "./cli-contract.js";
import { cliActivityTable, cliControlId, cliErrorMessage, cliLabelFor, cliTextInput, } from "./cli-components.js";
import { clear, el } from "./dom.js";
import { createWorkbenchOrientation, mountedWorkbench, } from "./workbench-chrome.js";
export function createCliIdleView(options) {
    const { draft } = options;
    const intro = el("section", { class: "att-idle-intro" }, [
        el("p", { class: "att-kicker" }, ["Connection"]),
        el("h2", {}, ["Attach one Senda CLI"]),
        el("p", {}, [
    ...
    const paintForm = () => {
        ...
        const addPairRows = (host, pairs, fallbackFocus, focusIndex) => {
            clear(host);
            for (const [index, pair] of pairs.entries()) {
                ...
                remove.addEventListener("click", () => {
                    ...
                    addPairRows(host, pairs, fallbackFocus, nextIndex >= 0 ? nextIndex : undefined);
                    if (nextIndex < 0)
                        fallbackFocus?.focus();
                });
        ...
        addEnvironment.addEventListener("click", () => {
            ...
            addPairRows(environmentHost, draft.environment, addEnvironment, draft.environment.length - 1);
        });
        ...
        ]), environmentHost, addEnvironment, el("div", { class: "att-actions" }, [connect]), feedback);
        ...
        const orientation = el("aside", { class: "att-idle-orient" }, [
            ...
            el("p", {}, [
                "It inspects an installed Senda CLI without loading an engine.",
            ]),
...
export function createCliUnavailableView(state) {
    const label = state.state === "connecting"
...
//# sourceMappingURL=cli-connection-view.js.map