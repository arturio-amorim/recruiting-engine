...
const tokensStorageKey = "senda-devtools.tokens";
const activeStorageKey = "senda-devtools.active";
/**
...
let activeKey = storedSelection === anonymousSelection ? null : storedSelection;
let selectionIsExplicit = storedSelection !== null;
let knownPrincipals = new Map();
const changeListeners = new Set();
function rememberPrincipals(principals) {
    knownPrincipals = new Map(principals.map((entry) => [entry.key, entry]));
}
function rememberPrincipal(principal) {
...
function notifyPrincipalChange() {
...
export function setActivePrincipal(key) {
    activeKey = key;
...
export function getActivePrincipalKey() {
    return activeKey;
}
...
export function getActivePrincipalStatus() {
    if (activeKey === null)
        return null;
    const activePrincipal = knownPrincipals.get(activeKey);
    if (activePrincipal === undefined)
        return null;
    return {
...
export function onPrincipalChange(listener) {
...
export function listKnownPrincipals() {
    return [...knownPrincipals.values()];
}
export async function ensureActiveToken() {
    ...
    const selected = principals.find(({ key }) => key === activeKey) ??
...
export function renderPrincipalsPanel(container) {
    let active = true;
    ...
    const clearPanelStatus = () => {
        panelStatus.textContent = "";
    };
    /** What one identity managed to do, newest first. */
    const recentContent = (principalId) => {
        ...
        return calls.map((call) => el("span", { class: "principal-recent-call" }, [
    ...
    const render = async () => {
        ...
        try {
            ...
            panelBody.append(heading, el("p", { class: "feedback", role: "alert" }, [
        ...
        const rows = principals.map((entry, index) => {
            ...
            const actionButtons = [];
            const setActionStatus = (message, kind = "neutral") => {
                actionStatus.setAttribute("class", kind === "error"
            ...
            const cancelConfirmation = el("button", {
                type: "button",
            ...
            const resetConfirmation = () => {
                ...
                remove.setAttribute("aria-label", `Delete test identity ${entry.principal.id}`);
                cancelConfirmation.hidden = true;
            };
            const beginConfirmation = (kind) => {
                ...
                if (kind === "rotation") {
                    if (hasToken) {
                        ...
                        tokenAction.setAttribute("aria-label", `Confirm token rotation for ${entry.principal.id}`);
                        setActionStatus(`Rotate the token for “${entry.principal.id}”? The current token will stop working immediately.`);
                    }
                    else {
                        tokenAction.textContent = "Confirm token creation";
                        tokenAction.setAttribute("aria-label", `Confirm session token creation for ${entry.principal.id}`);
                        setActionStatus(`Create a session token for “${entry.principal.id}” and use it for invocations?`);
                    }
                    return;
                }
                remove.textContent = "Confirm delete";
                remove.setAttribute("aria-label", `Confirm deletion of test identity ${entry.principal.id}`);
                setActionStatus(`Delete “${entry.principal.id}”? Its token will stop working immediately.`);
            };
            const issueToken = () => {
                clearPanelStatus();
                resetConfirmation();
                ...
                void api
                    ...
                    setActionStatus(hasToken
                        ...
                        : "The session token could not be created. No credential was added.", "error");
                });
            };
            ...
            remove.addEventListener("click", () => {
                ...
                void api
                    ...
                    setActionsDisabled(false);
                    resetConfirmation();
                    setActionStatus(`Test identity “${entry.principal.id}” could not be deleted. Try again.`, "error");
                });
            });
            const fillForm = (id) => {
                clearPanelStatus();
            ...
            actionButtons.push(tokenAction, edit, duplicate, remove, cancelConfirmation);
        ...
        const setCreateFeedback = (message, kind = "neutral") => {
            feedback.setAttribute("class", kind === "error"
                ? "principal-status feedback"
                : "principal-status hint");
        ...
        const clearFieldError = (field) => {
            ...
            if (idInput.getAttribute("aria-invalid") === "false" &&
        ...
        const readFormPrincipal = () => {
            ...
            return { id, attributes: parsed };
        };
        const create = el("button", { type: "button" }, ["Add identity"]);
        const cancelEdit = el("button", { type: "button", "aria-label": "Cancel editing this identity" }, ["Cancel"]);
        ...
        const saveEdit = (key, principal) => {
            ...
            void api
                ...
                setCreateFeedback(error instanceof Error && error.message !== ""
                    ? error.message
                    : "The test identity could not be updated. It is unchanged.", "error");
            });
        };
        create.addEventListener("click", () => {
            ...
            setCreateFeedback(`Adding “${principal.id}”, minting its token, and making it active…`);
            void api
                ...
                setCreateFeedback(error instanceof Error && error.message !== ""
                    ? error.message
                    : "The test identity could not be added. Try again.", "error");
            });
        });
        const heading = el("h2", { tabindex: "-1" }, ["Test identities"]);
        const identityCount = principals.length;
        const countLabel = `${String(identityCount)} test ${identityCount === 1 ? "identity" : "identities"}`;
        ...
        const createOutcome = el("span", { class: "hint principal-create-outcome" }, ["Mints a token and activates it"]);
        const createHint = el("p", { id: createHintId, class: "hint principal-create-hint" }, [
            "Create an in-memory Principal for development and homologation. Its bearer token stays hidden in this browser session.",
        ]);
        ...
        const identitiesSection = el("section", {
        ...
        panelBody.append(header, el("div", { id: tokenGuidanceId, class: "principal-token-guidance" }, [
        ...
        ]), identitiesSection, createSection);
    ...
    panelBody.append(el("h2", {}, ["Test identities"]), el("p", { class: "hint", role: "status" }, ["Loading test identities…"]));
    void render();
    ...
    const source = typeof EventSource === "function" ? new EventSource("/api/events") : null;
    source?.addEventListener("trace", (event) => {
        try {
            ...
            if (recentCalls.length > recentCallCapacity)
                recentCalls.pop();
            paintRecent();
        }
        catch {
