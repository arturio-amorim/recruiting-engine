...
export const workbenchChoices = [
    {
        workbench: "mcp",
        title: "MCP workbench",
        short: "Inspect an installed MCP server",
        summary: "Inspect an installed MCP server: list its tools, call one, and read the exchange.",
        connects: "Connects a stdio command or a Streamable HTTP URL.",
        flag: "senda-devtools open --mcp",
    },
    {
        workbench: "cli",
        title: "CLI workbench",
        short: "Inspect an installed Senda CLI",
        summary: "Inspect an installed Senda CLI: list its capabilities, describe one, and run it.",
        connects: "Connects an executable with its arguments, cwd, and environment.",
        flag: "senda-devtools open --cli",
    },
];
export function workbenchChoice(workbench) {
    return workbenchChoices.find((choice) => choice.workbench === workbench);
}
/**
...
function resolveDocument(ownerDocument) {
 ...
 * leaves the default.
 */
export function workbenchApiBase(ownerDocument) {
    const declared = resolveDocument(ownerDocument)?.body?.dataset.sendaApi;
 ...
 */
export function mountedWorkbench(ownerDocument) {
    const declared = resolveDocument(ownerDocument)?.body?.dataset.sendaWorkbench;
 ...
 */
export function createBrandLockup(current) {
    const parts = [
        createBrandMark(),
        el("span", { class: "att-brand-name" }, ["senda"]),
        el("span", { class: "att-product-name" }, ["DevTools"]),
    ];
    if (current === undefined)
        return el("div", { class: "att-brand" }, parts);
    return el("a", { class: "att-brand att-brand-link", href: "/", title: "All workbenches" }, parts);
}
/**
 ...
 */
export function createWorkbenchSwitch(current) {
    return el("nav", { class: "att-switch", "aria-label": "Workbench" }, [
        ...
        ...Object.keys(workbenchPaths).map((name) => el("a", {
...
function createOrientationLink(href, title, hint) {
 ...
 */
export function createWorkbenchOrientation(current) {
    if (current === undefined)
        return undefined;
    const peer = workbenchChoice(current === "mcp" ? "cli" : "mcp");
    return el("div", { class: "att-orient-links" }, [
        createOrientationLink(chooserPath, chooserLabel, "Choose which workbench to open"),
        createOrientationLink(workbenchPaths[peer.workbench], peer.title, peer.short),
    ]);
}
//# sourceMappingURL=workbench-chrome.js.map