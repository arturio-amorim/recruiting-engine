import { connectMcpClient, } from "@senda/mcp";
...
function resolveLimits(options) {
    return {
        initializationDeadlineMs: options.initializationDeadlineMs ?? defaultInitializationDeadlineMs,
...
function isVerificationErrorCode(value) {
    return (typeof value === "string" && Object.hasOwn(verificationErrorMessages, value));
}
...
function asFailure(error, stage, target) {
...
function limitExceeded(limit, value) {
    return {
        stage: "catalog",
...
async function withDeadline(deadlineMs, stage, callerSignal, operation, onLateValue) {
...
async function collectCatalog(connection, signal, limits) {
    let cursor;
    ...
    while (hasNextPage) {
        ...
        if (pageCount > limits.maxCatalogPages) {
            throw new LocalVerificationError("LIMIT_EXCEEDED", limitExceeded("maxCatalogPages", limits.maxCatalogPages));
        }
        ...
        for (const tool of page.tools) {
            ...
            if (toolCount > limits.maxTools) {
                throw new LocalVerificationError("LIMIT_EXCEEDED", limitExceeded("maxTools", limits.maxTools));
            }
            ...
            if (catalogBytes > limits.maxCatalogBytes) {
                throw new LocalVerificationError("LIMIT_EXCEEDED", limitExceeded("maxCatalogBytes", limits.maxCatalogBytes));
            }
        }
 ...
 */
export async function runMcpVerification(options) {
    ...
    try {
        ...
        stage = "initialize";
        connection = await withDeadline(limits.initializationDeadlineMs, "initialize", options.signal, (signal) => connect(options.target, { signal }), (lateConnection) => {
            void lateConnection.close().catch(() => undefined);
        });
        stage = "catalog";
        const catalog = await withDeadline(limits.catalogDeadlineMs, "catalog", options.signal, (signal) => collectCatalog(connection, signal, limits));
        stage = null;
        result = {
            ok: true,
            status: "ok",
 ...
 * should use.
 */
export function renderMcpVerificationResult(result) {
    if (result.ok) {
        const legacySuccess = {
    ...
    return {
        ...
        stderr: `senda-devtools verify: ${result.code}: ${result.message}\n`,
    };
}
