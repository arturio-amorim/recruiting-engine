...
import { isCapabilityCompositionError, isComposedCapabilities, } from "@senda/core";
const programName = "senda";
const defaultExportName = "capabilities";
const usage = `Usage:
  senda check-capabilities <esm-module> [--export <name>]
...
function renderDeclaration(declaration) {
    ...
    if (kind === "atomic") {
        ...
        const renderedVersion = version === undefined ? "" : ` sourceVersion=${token(version)}`;
...
function renderIssue(issue) {
    ...
    if (code === "CAPABILITY_IMPORT_ID_NOT_FOUND" ||
...
function orderIssues(issues) {
    ...
    for (const issue of issues) {
        const record = asRecord(issue);
        if (record?.code === "CAPABILITY_ID_COLLISION" &&
    ...
    collisions.sort((left, right) => compareCodeUnits(asRecord(left).effectiveId, asRecord(right).effectiveId));
...
function renderCompositionFailure(command, error) {
    const header = [
        `${programName}: the capability composition is invalid.`,
        ...renderContext(command),
    ];
    try {
        const issues = orderIssues((asRecord(error)?.issues ?? []));
        return renderLines([
            ...header,
...
function renderLoadFailure(command, error) {
    return renderLines([
...
function renderUntrackedExport(command) {
    return renderLines([
...
//# sourceMappingURL=check-capabilities.js.map