...
import { selectPackageManager, supportedLockfileNames, } from "./generate/lockfile.js";
import { planGeneratedFiles } from "./generate/plan.js";
...
async function detectPackageManager(cwd) {
    ...
    for (const lockfile of supportedLockfileNames) {
        if (await isRegularFile(join(cwd, lockfile)))
 ...
 * written to `stdout`.
 */
export async function runPackage(args, context) {
    if (args.length > 0) {
    ...
    try {
        ...
        if (!manifestResult.ok)
            throw toDeployError(manifestResult);
        ...
        if (conflicts.length > 0) {
            throw new DeployError("GENERATED_FILE_CONFLICT", {
                details: conflicts.map((file) => `${file.path}: the first line must be ${file.markerLine}`),
            });
        }
        return 0;
    }
