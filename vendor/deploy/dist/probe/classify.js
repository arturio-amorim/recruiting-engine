...
function unhealthy(status, reason) {
...
function parseProtocolMessage(response) {
 ...
 */
function isInitializeResult(response) {
 ...
 */
export function classifyProbeExchange(expect, exchange) {
    if (exchange.outcome === "timeout")
        return unreachable(probeReasons.TIMEOUT);
