...
function parseProbeUrl(value) {
    if (value.length === 0 || value.length > maxUrlLength)
        return undefined;
...
function parseExpectation(value) {
    if (value === undefined || value === "alive")
        return "alive";
 ...
 */
export function parseProbeOptions(args, env) {
    ...
    if (bearerEnv !== undefined) {
        ...
        if (expect !== "ready")
            return rejected;
        if (!environmentNamePattern.test(bearerEnv))
            return rejected;
