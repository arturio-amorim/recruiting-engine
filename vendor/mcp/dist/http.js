...
const MCP_PATH = "/mcp";
function snapshotChallengeScopes(value, hasResourceMetadata) {
    if (value === undefined)
        return undefined;
    ...
    if (!Array.isArray(value) || value.length === 0) {
        throw new TypeError("auth.challengeScopes must contain at least one scope token.");
    }
    const scopes = [];
    const seen = new Set();
    ...
    for (const scope of value) {
        if (typeof scope !== "string" ||
            ...
            throw new TypeError("auth.challengeScopes must contain only RFC 6749 scope tokens.");
        }
        if (seen.has(scope)) {
        ...
        if (serializedBytes > MAX_CHALLENGE_SCOPE_BYTES) {
            throw new TypeError(`auth.challengeScopes must serialize to at most ${MAX_CHALLENGE_SCOPE_BYTES} bytes.`);
        }
        scopes.push(scope);
    }
    return Object.freeze(scopes);
}
function snapshotAuthOptions(value) {
    if (value === null || typeof value !== "object") {
        throw new TypeError('auth.mode must be either "required" or "dangerously-disabled-for-development".');
    }
    if (value.mode === "required") {
        if (typeof value.authenticate !== "function") {
            throw new TypeError("auth.authenticate must be a function when auth.mode is required.");
        }
        const challengeScopes = snapshotChallengeScopes(value.challengeScopes, value.resourceMetadata !== undefined);
...
function bearerChallenge(metadataUrl, challengeScopes) {
...
function normalizeHostname(authority) {
    if (authority.toLowerCase() === "::1")
        return "[::1]";
    try {
...
function normalizeRequestHostname(authority) {
...
function normalizedAllowedHosts(bindHost, configured) {
    if (configured === undefined && !isLoopback(bindHost)) {
        throw new TypeError("allowedHosts is required when the MCP HTTP server binds to a non-loopback host.");
    }
...
function normalizedAllowedOrigins(configured) {
...
function toMetadata(metadata) {
    if (metadata.authorizationServers.length === 0) {
        throw new TypeError("Protected resource metadata requires at least one authorization server.");
    }
...
function validateResourceUrl(value) {
    ...
    try {
        ...
        throw new TypeError("Protected resource metadata has an invalid resource URL.");
    }
    const secure = url.protocol === "https:";
    const loopbackDevelopment = url.protocol === "http:" && isLiteralLoopback(url.hostname);
    if ((!secure && !loopbackDevelopment) ||
        ...
        url.search !== "" ||
        url.hash !== "") {
        throw new TypeError("Protected resource metadata requires an HTTPS /mcp resource URL, except for literal-loopback (127.0.0.1 or [::1]) HTTP development.");
    }
}
function validateAuthorizationServerUrl(value, resource) {
    let url;
    try {
        url = new URL(value);
    }
    catch {
        throw new TypeError("Protected resource metadata has an invalid authorization server URL.");
    }
    ...
    const loopbackDevelopment = url.protocol === "http:" &&
    ...
    if ((url.protocol !== "https:" && !loopbackDevelopment) ||
        ...
        url.search !== "" ||
        url.hash !== "") {
        throw new TypeError("Authorization server URLs require HTTPS, except on literal loopback behind a literal-loopback HTTP resource, and cannot contain credentials, a query, or a fragment.");
    }
}
...
function sendBeforeBodyConsumption(request, response, value) {
...
function requestUrl(hostHeader, path) {
...
function acceptableMediaType(value, expected) {
    if (value === undefined)
        return false;
...
function rawHeaderCount(rawHeaders, expectedName) {
    let count = 0;
...
function declaredBodyExceedsLimit(request, maxBytes) {
...
function sendPayloadTooLarge(request, response) {
    sendBeforeBodyConsumption(request, response, {
        status: 413,
        body: { error: "payload_too_large" },
    });
}
function sendProtocolError(response, status, code, message) {
    send(response, {
        status,
        body: {
            jsonrpc: "2.0",
...
async function writeWebResponse(target, source) {
    const headers = {};
...
export async function serveMcpHttp(engine, options) {
    ...
    const metadata = auth.mode === "required" && auth.resourceMetadata !== undefined
        ? toMetadata(auth.resourceMetadata)
        : undefined;
    const metadataUrl = metadata === undefined
        ? undefined
        : getOAuthProtectedResourceMetadataUrl(new URL(metadata.resource));
    const metadataPath = metadataUrl === undefined ? undefined : new URL(metadataUrl).pathname;
    ...
    const httpServer = createServer((request, response) => {
        void (async () => {
            ...
            if (auth.mode === "required") {
                ...
                if (authenticatedSnapshot === null) {
                    sendBeforeBodyConsumption(request, response, {
                        status: 401,
                        headers: {
                            "www-authenticate": bearerChallenge(metadataUrl, auth.challengeScopes),
                        },
            ...
            if (rawHeaderCount(request.rawHeaders, "content-type") !== 1 ||
            ...
            const statelessTransportOptions = {
                enableJsonResponse: true,
            };
            const transport = new WebStandardStreamableHTTPServerTransport(statelessTransportOptions);
            ...
            try {
                ...
                if (Array.isArray(parsedBody)) {
                    sendProtocolError(response, 400, -32600, "Invalid Request: Exactly one JSON-RPC message is required");
                    return;
                }
                ...
                if (!response.headersSent && !response.destroyed) {
                    ...
                    if (consumedBody)
                        ...
                        sendBeforeBodyConsumption(request, response, failure);
                }
            }
            finally {
