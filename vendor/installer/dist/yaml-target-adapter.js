...
function inspectYamlAst(contents, serverName) {
    ...
    while (stack.length > 0) {
        ...
        if (task.kind === "finish-map") {
            const inspected = inspectedJsonRecord(task.fields, selectedPath(task.path, serverPath), samePath(task.path, serverPath));
            values.set(task.node, inspected);
            task.assign(inspected);
            continue;
        }
        if (task.kind === "finish-sequence") {
            const inspected = inspectedJsonArray(task.items, selectedPath(task.path, serverPath));
            values.set(task.node, inspected);
            task.assign(inspected);
            continue;
        }
        ...
        if (isScalar(node)) {
            ...
            const inspected = inspectedJsonScalar(scalar, selectedPath(path, serverPath));
            values.set(node, inspected);
            assign(inspected);
            continue;
        }
        if (isMap(node)) {
            const restrictedTagPath = path.length === 0 ||
            ...
            for (let index = node.items.length - 1; index >= 0; index -= 1) {
                ...
                if (pair.key.tag === "tag:yaml.org,2002:merge" ||
                    (key === "<<" &&
                        pair.key.type === "PLAIN" &&
                        pair.key.tag === undefined)) {
                    invalid();
                }
                if (key === undefined || objectMembers.has(key))
                    invalid();
                ...
                const normalizedKey = samePath(path, [...serverPath, "headers"])
                    ...
                    : key;
                if (pair.value === null) {
                    const inspected = inspectedJsonScalar(null, selectedPath([...path, key], serverPath));
            ...
            const items = new Array(node.items.length);
            ...
            for (let index = node.items.length - 1; index >= 0; index -= 1) {
                ...
                if (isNode(item)) {
                    ...
                    if (item.key.tag === "tag:yaml.org,2002:merge" ||
                        (key === "<<" &&
                            item.key.type === "PLAIN" &&
                    ...
                    if (item.value === null) {
                        ...
                        stack.push({
                            kind: "visit",
                            node: item.value,
                            depth: isMap(item.value) || isSeq(item.value)
                                ? pairDepth + 1
                                : pairDepth,
                            path: [...path, key],
                            assign: (assigned) => {
                    ...
                    items[index] = inspectedJsonScalar(null, selectedPath(path, serverPath));
                }
                else
                    invalid();
            }
...
function mapValue(members, map, key) {
...
function parseAndInspect(sourceBytes, serverName, counters, phase, inspectionOwner) {
    ...
    if (document.errors.length > 0 || document.warnings.length > 0)
        invalid();
    const emptyContents = document.contents === null ||
        (isScalar(document.contents) &&
            document.contents.value === null &&
            document.contents.range?.[0] === document.contents.range?.[1]);
    if (emptyContents) {
        const documentValueEnd = document.contents?.range?.[1] ?? document.range?.[1] ?? 0;
    ...
    const serverPair = parent === undefined
        ? undefined
        : astInspection.members.get(parent)?.get(serverName);
    const server = serverPair === undefined
        ? undefined
    ...
    const finalized = inspectedServer?.kind === "record"
        ...
        : undefined;
    const enabledCandidate = server === undefined
        ? undefined
    ...
    const enabled = enabledCandidate;
    if (phase === "source")
        inspectionPass(counters);
...
function renderDefinitionLines(definition, indent) {
    const lines = [];
    if (typeof definition.command === "string" &&
        Array.isArray(definition.args)) {
        lines.push(`${indent}command: ${yamlString(definition.command)}`);
        lines.push(`${indent}args: [${definition.args
            .map((argument) => yamlString(argument))
            .join(", ")}]`);
        if (typeof definition.env === "object" &&
            definition.env !== null &&
        ...
        if (typeof definition.headers === "object" &&
            ...
            for (const [name, value] of Object.entries(definition.headers)) {
                ...
                lines.push(`${indent}  ${yamlString(renderedName)}: ${yamlString(value)}`);
            }
        }
    }
    else
...
function renderServerBlock(serverName, definition, keyIndent, newline) {
    return [
        `${keyIndent}${serverName}:`,
        ...renderDefinitionLines(definition, `${keyIndent}  `),
    ].join(newline);
}
function yamlConfigDefinition(definition) {
    const { transport: _transport, ...configDefinition } = definition;
    if (typeof configDefinition.headers !== "object" ||
        configDefinition.headers === null ||
        Array.isArray(configDefinition.headers)) {
        if (typeof configDefinition.env === "object" &&
    ...
    const withHeaders = {
        ...configDefinition,
        headers: Object.fromEntries(Object.entries(configDefinition.headers).map(([name, value]) => [
...
function insertFlowProperty(text, map, key, value) {
    ...
    if (lastRange === undefined || lastRange === null)
        invalid();
    const sourceToken = map.srcToken;
    const trailingItem = sourceToken?.type === "flow-collection"
        ? sourceToken.items[map.items.length]
        : undefined;
    const hasTrailingComma = trailingItem?.start.some((token) => token.type === "comma") ?? false;
    const separator = hasTrailingComma ? " " : ", ";
    return `${text.slice(0, close)}${separator}${JSON.stringify(key)}: ${value}${text.slice(close)}`;
}
function insertBlockEntry(source, map, block) {
    const range = map.range;
    if (range === undefined || range === null)
        invalid();
...
function appendRootBlock(source, block) {
...
function insertEmptyDocumentBlock(source, offset, block) {
...
function pairRange(pair) {
    if (!isNode(pair.key) || !isNode(pair.value))
        invalid();
...
function removePair(text, parent, pair) {
    ...
    if (index < parent.items.length - 1) {
        const [nextStart] = pairRange(parent.items[index + 1]);
...
function constructPatch(request, inspectionOwner) {
    ...
    if (request.action === "install") {
        const block = [
            "mcp_servers:",
            renderServerBlock(state.serverName, request.definition, "  ", state.source.newline),
        ].join(state.source.newline);
        if (state.source.missing) {
            postText = appendRootBlock(state.source, block);
        }
        else if (state.root === undefined) {
            postText = insertEmptyDocumentBlock(state.source, state.emptyInsertionOffset ?? state.source.text.length, block);
        }
        else if (state.parent === undefined && state.root.flow) {
            postText = insertFlowProperty(state.source.text, state.root, "mcp_servers", JSON.stringify({
            ...
            postText = insertFlowProperty(state.source.text, state.parent, state.serverName, JSON.stringify(yamlConfigDefinition(request.definition)));
        }
        else {
            const firstPair = state.parent.items[0];
            const keyRange = firstPair !== undefined && isNode(firstPair.key)
                ? firstPair.key.range
                : undefined;
            const keyIndent = keyRange === undefined || keyRange === null
                ? "  "
                : lineIndent(state.source.text, keyRange[0]);
            postText = insertBlockEntry(state.source, state.parent, renderServerBlock(state.serverName, request.definition, keyIndent, state.source.newline));
        }
    }
    else if (request.action === "remove") {
        ...
        else if (state.server.flow) {
            postText = insertFlowProperty(state.source.text, state.server, "enabled", String(desired));
        }
        else {
            const firstPair = state.server.items[0];
            const keyRange = firstPair !== undefined && isNode(firstPair.key)
                ? firstPair.key.range
                : undefined;
            const indent = keyRange === undefined || keyRange === null
                ? "    "
                : lineIndent(state.source.text, keyRange[0]);
            postText = insertBlockEntry(state.source, state.server, `${indent}enabled: ${String(desired)}`);
        }
    }
    const postImage = encodeTargetPostImage(postText, state.source.bom, request.counters);
    const postInspection = parseAndInspect(postImage, state.serverName, request.counters, "post-image", inspectionOwner);
...
export function createYamlTargetAdapter(options) {
    ...
    return Object.freeze({
        ...
        inspect: ({ source, serverName, counters }) => parseAndInspect(source, serverName, counters, "source", inspectionOwner),
        constructPatch: (request) => constructPatch(request, inspectionOwner),
    });
}
export function yamlDefinition(definition) {
    return freezeDefinition(definition);
}
//# sourceMappingURL=yaml-target-adapter.js.map