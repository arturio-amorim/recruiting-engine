...
import { chmod, lstat, mkdir, open, readFile, realpath, rename, rmdir, unlink, } from "node:fs/promises";
import { InstallerFileSystemError, maximumInstallerFileBytes, } from "./file-system.js";
import { captureProcessOwnershipIdentity, enforcesPosixFileModes, } from "./ownership-identity.js";
const readChunkBytes = 65_536;
/** Windows exposes none of these open flags; a missing flag contributes zero. */
const platformOpenFlags = constants;
...
function normalizeFileSystemError(error) {
    if (error instanceof InstallerFileSystemError)
        return error;
...
async function normalized(operation) {
    try {
...
function safeMetadataNumber(value) {
...
function verifyCreatedOwner(ownership, created) {
...
function sameObjectIdentity(expected, current) {
...
async function lstatIdentity(path) {
    try {
...
async function closeAfterSetupFailure(handle) {
    try {
        await handle.close();
    }
    catch {
...
async function unlinkCreatedFile(path, created) {
...
async function removeCreatedDirectory(path, created) {
    const current = await lstatIdentity(path);
    if (current === undefined ||
...
async function handleStat(handle) {
    return normalized(async () => toFileStat(await handle.stat({ bigint: true })));
}
    ...
    return Object.freeze({
        readAll: async (maxBytes) => {
            ...
            return normalized(async () => {
                ...
                let total = 0;
                while (true) {
                    const requested = total === maxBytes ? 1 : Math.min(readChunkBytes, maxBytes - total);
 ...
 */
async function openReadNoFollowEmulated(path) {
    ...
    const handle = await open(path, constants.O_RDONLY | noFollowFlag | nonBlockFlag);
    try {
        const opened = await handleStat(handle);
        if (opened.kind !== before.kind ||
...
export function createNodeFileSystem(options = {}) {
    ...
    const strictPosixModes = ownership === undefined || enforcesPosixFileModes(ownership);
    ...
    return {
        ...
        openReadNoFollow: async (path) => normalized(async () => readHandle(emulateNoFollow
            ? await openReadNoFollowEmulated(path)
            : await open(path, constants.O_RDONLY | noFollowFlag | nonBlockFlag))),
        createExclusiveNoFollow: async (path, mode) => {
            ...
            return normalized(async () => {
                ...
                try {
                    ...
                    if (!sameObjectIdentity(created, configured) ||
        ...
        mkdir: async (path, mode) => {
            ...
            await normalized(async () => {
                ...
                try {
                    ...
                    if (strictPosixModes) {
                        const handle = await open(path, constants.O_RDONLY | directoryFlag | noFollowFlag | nonBlockFlag);
                        try {
                            const descriptor = await handleStat(handle);
                            if (!sameObjectIdentity(created, descriptor) ||
