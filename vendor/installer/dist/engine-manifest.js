...
function validString(value, options = {}) {
...
function validUniqueStringArray(value, options) {
    ...
    for (const item of value) {
        if (!validString(item, { nonempty: true }) || seen.has(item))
            return false;
...
function validEntrypoint(value) {
    if (!validString(value, { limit: entrypointScalarLimit, nonempty: true }) ||
        value.startsWith("/") ||
        value.includes("\\")) {
        return false;
    }
    const segments = value.split("/");
    return (segments.length <= entrypointSegmentLimit &&
        segments.every((segment) => segment !== "" && segment !== "." && segment !== ".."));
}
function hasDuplicateObjectKey(node) {
    if (node.type === "Object") {
        const names = new Set();
        for (const member of node.members) {
            const name = member.name.type === "String" ? member.name.value : member.name.name;
            if (names.has(name) || hasDuplicateObjectKey(member.value))
                return true;
            names.add(name);
        }
    }
    else if (node.type === "Array") {
        return node.elements.some((element) => hasDuplicateObjectKey(element.value));
    }
    return false;
}
function validateParsedManifest(value) {
    if (!isRecord(value) ||
...
export function validateEngineInstallManifestBytes(bytes) {
    ...
    try {
        ...
        if (hasDuplicateObjectKey(document.body))
            return manifestInvalid();
...
async function readCapturedManifest(fileSystem, identity) {
    let handle;
    try {
...
async function loadOwnedEngineManifest(options) {
    ...
    try {
        ...
        manifestIdentity = await capturePathIdentity(options.fileSystem, {
            root,
            targetPath: join(projectDirectory, "senda.mcp.json"),
    ...
    const manifest = validateEngineInstallManifestBytes(await readCapturedManifest(options.fileSystem, manifestIdentity));
    return Object.freeze({
        manifest,
...
export async function loadEngineRemovalManifest(options) {
...
export async function loadEngineInstallManifest(options) {
    if (!isAbsolute(options.nodeExecutable) ||
        options.nodeExecutable.includes("\0")) {
        throw new InstallerError("ENGINE_PATH_UNSAFE");
    }
    const loaded = await loadOwnedEngineManifest(options);
    const { manifest, projectDirectory, root } = loaded;
    const entrypointPath = join(projectDirectory, ...manifest.server.entrypoint.split("/"));
    let entrypointIdentity;
    try {
...
//# sourceMappingURL=engine-manifest.js.map