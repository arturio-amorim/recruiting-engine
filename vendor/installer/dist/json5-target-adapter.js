...
function inspectAst(document, serverName) {
    ...
    while (stack.length > 0) {
        ...
        const inspected = inspectedJsonScalar(scalarValue(node), selectedPath(path, serverPath));
...
function hasTrailingComma(state, object) {
    const last = object.members.at(-1);
    if (last === undefined)
        return false;
    const [, lastEnd] = range(last);
    const [, objectEnd] = range(object);
    return state.tokens.some((token) => token.type === "Comma" &&
        range(token)[0] >= lastEnd &&
        range(token)[1] < objectEnd);
}
function insertProperty(text, state, object, key, value, newline) {
...
function jsonConfigDefinition(definition) {
    const isStdio = definition.transport === "stdio";
    const configDefinition = isStdio
        ? Object.fromEntries(Object.entries(definition).filter(([key]) => key !== "transport"))
        : { ...definition };
    ...
    configDefinition.headers = Object.fromEntries(Object.entries(configDefinition.headers).map(([name, value]) => [
        name === "authorization" ? "Authorization" : name,
        value,
    ]));
    return configDefinition;
}
function jsonDefinition(definition) {
...
function constructPatch(request, inspectionOwner) {
    ...
    if (request.action === "install") {
        ...
        if (source.missing) {
            ...
            postText = insertProperty(source.text, state, state.root, "mcp", `{"servers":{${JSON.stringify(state.serverName)}:${entry}}}`, source.newline);
        }
        else if (state.servers === undefined) {
            postText = insertProperty(source.text, state, state.mcp, "servers", `{${JSON.stringify(state.serverName)}:${entry}}`, source.newline);
        }
        else {
            postText = insertProperty(source.text, state, state.servers, state.serverName, entry, source.newline);
        }
    }
    else if (request.action === "remove") {
        ...
        postText =
            state.enabled === undefined
                ? insertProperty(source.text, state, state.server, "enabled", String(desired), source.newline)
...
export function json5Definition(definition) {
    return freezeDefinition(definition);
}
//# sourceMappingURL=json5-target-adapter.js.map