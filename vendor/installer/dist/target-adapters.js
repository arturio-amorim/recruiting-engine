...
import { createJsonTargetAdapter, jsonDefinition, } from "./json-target-adapter.js";
import { createJson5TargetAdapter, json5Definition, } from "./json5-target-adapter.js";
import { createTargetAdapterCounters, } from "./target-adapter.js";
import { createTomlTargetAdapter, tomlDefinition, } from "./toml-target-adapter.js";
import { createYamlTargetAdapter, yamlDefinition, } from "./yaml-target-adapter.js";
export { createTargetAdapterCounters };
export const openClawEnvironmentPolicyCommit = "f308af8a344a30432e1b13fa348533e54cd190c8";
...
const openClawDeniedEnvironmentNames = new Set(openClawDeniedEnvironmentNameSnapshot);
function openClawDeniedEnvironment(name) {
    const normalized = name.trim().toUpperCase();
    return (openClawDeniedEnvironmentNames.has(normalized) ||
        normalized.startsWith("BASH_FUNC_") ||
        normalized.startsWith("DYLD_") ||
        normalized.startsWith("LD_"));
}
function portableCompatibility() {
    return { supported: true };
}
function validPortableMcpUrl(value) {
    let parsed;
    try {
...
function invalidDefinitionInverse() {
    throw new InstallerError("HARNESS_CONFIG_INVALID");
}
function inverseRecord(value) {
    try {
...
function inverseEntries(record) {
    const entries = [];
    let keys;
    try {
...
function inverseOwn(record, key) {
    let property;
    try {
...
function inverseOptionalOwn(record, key) {
    let property;
    try {
        property = Object.getOwnPropertyDescriptor(record, key);
    }
...
function inverseShape(record, allowed, required = allowed) {
    const keys = inverseEntries(record).map(([key]) => key);
    const allowedKeys = new Set(allowed);
    if (keys.some((key) => !allowedKeys.has(key)) ||
        required.some((key) => Object.getOwnPropertyDescriptor(record, key) === undefined)) {
        invalidDefinitionInverse();
    }
}
function inverseString(record, key) {
    const value = inverseOwn(record, key);
    if (typeof value !== "string")
        return invalidDefinitionInverse();
    return value;
}
function inverseOptionalEnvironmentName(record, key) {
...
function inverseStringArray(record, key) {
    const value = inverseOwn(record, key);
    ...
    if (keys.length !== value.length + 1 ||
        ...
        keys.some((arrayKey) => arrayKey !== "length" &&
...
function barePlaceholder(name, style) {
    return style === "plain"
        ? `\${${name}}`
        : style === "cursor"
            ? `\${env:${name}}`
...
function inverseEnvironmentObject(value, style) {
    const record = inverseRecord(value);
    const result = [];
...
function placeholderEnvironmentName(value, style, bearer) {
    if (typeof value !== "string")
        return invalidDefinitionInverse();
...
function inversePlaceholderHeaders(value, style) {
    const record = inverseRecord(value);
    const headers = {};
    let bearerVariable;
    for (const [name, placeholder] of inverseEntries(record)) {
    ...
    return Object.freeze({
        authentication: bearerVariable === undefined
...
function inverseDirectHeaders(value) {
    const record = inverseRecord(value);
    const headers = {};
...
function inverseToggle(record, key) {
    if (typeof inverseOwn(record, key) !== "boolean") {
        invalidDefinitionInverse();
    }
}
function frozenStdioTransport(command, args, forwardEnv) {
    return Object.freeze({
        type: "stdio",
        command,
...
function frozenHttpTransport(url, authentication, headersFromEnv) {
    return Object.freeze({
...
function definitionToSuspendedDescriptor(targetId, serverName, definition) {
    try {
        ...
        if (transport === "stdio") {
            ...
            if (targetId === "opencode-v2") {
                ...
                forwardEnv = inverseEnvironmentObject(inverseOwn(record, "environment"), "opencode");
                inverseToggle(record, "disabled");
            }
            else {
                const environmentField = targetId === "codex"
                    ? "env_vars"
                    : targetId === "antigravity" || targetId === "kimi-code"
                        ? undefined
                        : "env";
                const typeField = targetId === "claude-code" ||
                    targetId === "claude-desktop" ||
                    targetId === "vscode"
                    ? "type"
                    : undefined;
                const toggleField = targetId === "claude-code" ||
                    targetId === "claude-desktop" ||
                    targetId === "cursor" ||
                    targetId === "vscode"
                    ? undefined
                ...
                if (environmentField === undefined) {
                    ...
                    if (forwardEnv.some((name) => !environmentNamePattern.test(name)) ||
                        new Set(forwardEnv).size !== forwardEnv.length) {
                        return invalidDefinitionInverse();
                    }
                }
                else {
                    forwardEnv = inverseEnvironmentObject(inverseOwn(record, environmentField), targetId === "cursor" || targetId === "vscode" ? "cursor" : "plain");
                }
                if (toggleField !== undefined) {
                    inverseToggle(record, toggleField);
                }
            }
            ...
            if (targetId === "antigravity") {
                ...
                const bearer = inverseOptionalEnvironmentName(record, "bearer_token_env_var");
                authentication =
                    bearer === undefined
                        ? Object.freeze({ type: "none" })
                        : Object.freeze({ type: "bearer-env", variable: bearer });
                headersFromEnv = inverseDirectHeaders(inverseOwn(record, "env_http_headers"));
                if (authentication.type === "bearer-env" &&
                ...
                inverseShape(record, ["transport", "url", "bearerTokenEnvVar", "enabled"], ["transport", "url", "enabled"]);
                url = inverseString(record, "url");
                const bearer = inverseOptionalEnvironmentName(record, "bearerTokenEnvVar");
                authentication =
                    bearer === undefined
                        ? Object.freeze({ type: "none" })
                        : Object.freeze({ type: "bearer-env", variable: bearer });
                ...
                if (oauthField !== undefined &&
                    inverseOwn(record, oauthField) !== false) {
                    return invalidDefinitionInverse();
                }
                url = inverseString(record, "url");
                const headers = inversePlaceholderHeaders(inverseOwn(record, headersField), targetId === "cursor" || targetId === "vscode"
                    ? "cursor"
                    : targetId === "opencode-v2"
                        ? "opencode"
                        : "plain");
            ...
            if (!validPortableMcpUrl(url))
                return invalidDefinitionInverse();
            canonicalTransport = frozenHttpTransport(url, authentication, headersFromEnv);
        }
        else {
            return invalidDefinitionInverse();
        }
...
function codexDefinition(descriptor) {
    ...
    return tomlDefinition({
        ...
        ...(transport.authentication.type === "bearer-env"
            ...
            : {}),
        env_http_headers: Object.fromEntries(Object.entries(transport.headersFromEnv).map(([name, environment]) => [
            name.toLowerCase(),
            environment,
        ])),
        enabled: true,
...
function placeholderHeaders(descriptor) {
    const transport = descriptor.server.transport;
...
function cursorPlaceholderHeaders(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type !== "streamable-http")
        return {};
    const headers = {};
...
function openCodePlaceholderHeaders(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type !== "streamable-http")
        return {};
    const headers = {};
...
function hermesDefinition(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "stdio") {
        const env = Object.fromEntries(transport.forwardEnv.map((name) => [name, `\${${name}}`]));
...
function openClawCompatibility(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "stdio") {
...
function openClawDefinition(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "stdio") {
        for (const name of transport.forwardEnv) {
            if (openClawDeniedEnvironment(name)) {
...
function claudeDefinition(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "stdio") {
        return jsonDefinition({
            ...
            command: transport.command,
            args: [...transport.args],
            env: Object.fromEntries(transport.forwardEnv.map((name) => [name, `\${${name}}`])),
        }, "detached");
    }
    return jsonDefinition({
        transport: "streamable-http",
...
function claudeDesktopCompatibility(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "streamable-http") {
        return {
            supported: false,
...
function cursorDefinition(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "stdio") {
        return jsonDefinition({
            transport: "stdio",
            command: transport.command,
            args: [...transport.args],
            env: Object.fromEntries(transport.forwardEnv.map((name) => [name, `\${env:${name}}`])),
        }, "detached");
    }
    return jsonDefinition({
        transport: "streamable-http",
...
function vscodeDefinition(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "stdio") {
        return jsonDefinition({
            transport: "stdio",
...
function antigravityCompatibility(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "stdio") {
        return transport.forwardEnv.length === 0
            ? { supported: true }
...
function antigravityDefinition(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "stdio") {
...
function kimiCompatibility(descriptor) {
    const transport = descriptor.server.transport;
...
function kimiDefinition(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "stdio") {
        if (transport.forwardEnv.length > 0) {
            throw new InstallerError("TARGET_UNSUPPORTED");
...
function openCodeDefinition(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "stdio") {
        return jsonDefinition({
            transport: "stdio",
            ...
            environment: Object.fromEntries(transport.forwardEnv.map((name) => [name, `{env:${name}}`])),
            disabled: false,
        }, "native-disabled");
    }
    return jsonDefinition({
...
function grokDefinition(descriptor) {
    const transport = descriptor.server.transport;
    if (transport.type === "stdio") {
        return tomlDefinition({
            transport: "stdio",
...
const codex = createTomlTargetAdapter({
    ...
    definitionToSuspendedDescriptor: (serverName, definition) => definitionToSuspendedDescriptor("codex", serverName, definition),
});
const hermes = createYamlTargetAdapter({
    compatibility: portableCompatibility,
    descriptorToDefinition: hermesDefinition,
    definitionToSuspendedDescriptor: (serverName, definition) => definitionToSuspendedDescriptor("hermes", serverName, definition),
});
const openclaw = createJson5TargetAdapter({
    compatibility: openClawCompatibility,
    definitionToSuspendedDescriptor: (serverName, definition) => definitionToSuspendedDescriptor("openclaw", serverName, definition),
...
const antigravity = createJsonTargetAdapter({
    ...
    definitionToSuspendedDescriptor: (serverName, definition) => definitionToSuspendedDescriptor("antigravity", serverName, definition),
    descriptorToDefinition: (descriptor) => {
...
const claudeCode = createJsonTargetAdapter({
    ...
    definitionToSuspendedDescriptor: (serverName, definition) => definitionToSuspendedDescriptor("claude-code", serverName, definition),
});
const claudeDesktop = createJsonTargetAdapter({
    ...
    definitionToSuspendedDescriptor: (serverName, definition) => definitionToSuspendedDescriptor("claude-desktop", serverName, definition),
});
const vscode = createJsonTargetAdapter({
    ...
    definitionToSuspendedDescriptor: (serverName, definition) => definitionToSuspendedDescriptor("vscode", serverName, definition),
});
const cursor = createJsonTargetAdapter({
    ...
    definitionToSuspendedDescriptor: (serverName, definition) => definitionToSuspendedDescriptor("cursor", serverName, definition),
});
const kimiCode = createJsonTargetAdapter({
    ...
    definitionToSuspendedDescriptor: (serverName, definition) => definitionToSuspendedDescriptor("kimi-code", serverName, definition),
    descriptorToDefinition: (descriptor) => {
...
const openCode = createJsonTargetAdapter({
    ...
    definitionToSuspendedDescriptor: (serverName, definition) => definitionToSuspendedDescriptor("opencode-v2", serverName, definition),
});
const grokBuild = createTomlTargetAdapter({
    ...
    definitionToSuspendedDescriptor: (serverName, definition) => definitionToSuspendedDescriptor("grok-build", serverName, definition),
});
...
export const registryCompatibilityAdapters = Object.freeze({
...
//# sourceMappingURL=target-adapters.js.map