...
import { inspectManagedInstallations, } from "./managed-installations.js";
import { mutateDescriptorAcrossTargets, } from "./mutation-coordinator.js";
import { resolveRuntimeRequirements } from "./runtime-requirements.js";
function cancel(prompter) {
    prompter.cancel(installerErrorMessages.CANCELLED);
    return 130;
}
function statusLine(view, runtime) {
...
async function showStatus(options, views) {
    if (views.length === 0) {
        options.prompter.note("No managed MCP installations.", "Status");
        options.prompter.outro("Nothing is installed by Senda.");
        return 0;
    }
    const lines = [];
...
export async function runManagementSession(options) {
    ...
    if (options.action === "status")
        return showStatus(options, views);
    const candidates = views.filter((view) => view.descriptor !== undefined &&
    ...
    const choices = new Map(candidates.map((view, index) => [String(index), view]));
    ...
    if (selection.kind === "cancelled")
        return cancel(options.prompter);
    ...
    const confirmation = await options.prompter.confirm(`${options.action === "enable" ? "Enable" : options.action === "disable" ? "Disable" : "Remove"} ${selected.installation.serverName} in ${selected.displayName}?`);
    ...
    if (result === undefined || result.outcome === "failed") {
        throw new InstallerError(result?.outcome === "failed" ? result.code : "INSTALLATION_UNAVAILABLE");
    }
    options.prompter.outro(`${selected.installation.serverName} ${result.outcome}. Reload ${selected.displayName}.`);
    return 0;
}
//# sourceMappingURL=management-session.js.map