...
import { installDescriptorAcrossTargets, } from "./mutation-coordinator.js";
...
export async function runInstallSession(options) {
    ...
    if (ready.kind === "blocked")
        throw new InstallerError(ready.code);
    const targets = options.snapshot.targets.filter((target) => target.eligible &&
        target.configuration.kind !== "blocked" &&
        options.dependencies.adapters[target.id].compatibility(options.descriptor)
            .supported);
    if (targets.length === 0)
        throw new InstallerError("NO_SUPPORTED_HARNESS");
    ...
    const confirmed = await options.prompter.confirm(`Update ${String(selection.value.length)} MCP client configuration${selection.value.length === 1 ? "" : "s"}?`);
    ...
    for (const result of results) {
        ...
        if (result.outcome === "failed") {
            failed = true;
            options.prompter.log("error", `${name}: ${result.code}: ${installerErrorMessages[result.code]}`);
        }
        else {
            options.prompter.log("success", `${name}: ${result.outcome === "installed" ? "installed" : "already installed"}.`);
        }
    }
...
//# sourceMappingURL=install-session.js.map